...
(function () {
  // Helper: extract last "url(...)" from background-image and return the file name
  function getFileNameFromBackground(bg) {
    if (!bg || bg === 'none') return '';
    // Some computed styles can contain multiple url(...) entries; pick the last one
    const urls = [...bg.matchAll(/url\((['"]?)(.*?)\1\)/gi)].map(m => m[2]);
    const raw = urls.length ? urls[urls.length - 1] : '';
    if (!raw) return '';
    const clean = raw.split('?')[0].split('#')[0]; // strip query/hash
  ...
  document.querySelectorAll('.ad-card').forEach(card => {
    ...
    // Insert before and move the card inside
    card.parentNode.insertBefore(wrap, card);
    wrap.appendChild(card);

    // Determine caption text: filename from bg or fallback to data attribute
    const bg = getComputedStyle(card).backgroundImage;
    const fname = getFileNameFromBackground(bg) || card.getAttribute('data-image-name') || '';

    const caption = document.createElement('div');
    caption.className = 'ad-caption';
    caption.textContent = fname || '—'; // show dash if no name
